...
uint8_t TX_ADDRESS[TX_ADR_WIDTH]={0x1e,0x3c,0x5a,0x78,0x96}; //发送地址
uint8_t RX_ADDRESS[RX_ADR_WIDTH]={0x1e,0x3c,0x5a,0x78,0x96}; //发送地址
...
//  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11|GPIO_Pin_12;	//PG6 7 推挽 	 
...
//	NRF24L01_CSN_1;			//SPI片选取消 
	NRF24L01_CE_0; 			//使能24L01

//	NRF24L01_Write_Reg(WRITE_REG_NRF+SETUP_AW,0x01);  //设置地址为3个字节长度
	...
	NRF24L01_Write_Reg(WRITE_REG_NRF+RX_PW_P0, RX_PLOAD_WIDTH);
  	...
  	NRF24L01_Write_Reg(WRITE_REG_NRF+SETUP_RETR,0xaa);//设置自动重发间隔时间:500us + 86us;最大自动重发次数:10次
...
u8 NRF24L01_Check(void)
{
	...
	NRF24L01_Write_Buf(WRITE_REG_NRF+TX_ADDR,(u8*)TX_ADDRESS,TX_ADR_WIDTH);//写入5个字节的地址.	
	NRF24L01_Read_Buf(TX_ADDR,buf,TX_ADR_WIDTH); //读出写入的地址  
	for(i=0;i<TX_ADR_WIDTH;i++)if(buf[i]!=TX_ADDRESS[i])return 0;//检测24L01错误	 							   	
	return 1;		 //检测到24L01
}	 	 
//SPI写寄存器
//reg:指定寄存器地址
...
//	LCD_Display_String(168,1,GREEN,BLUE, "R_Mode");
  	NRF24L01_Write_Reg(WRITE_REG_NRF+CONFIG, 0x3f);//配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式 
