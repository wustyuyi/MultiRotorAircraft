...
int16_t AccXOffset = -497;
int16_t AccYOffset = 112;
int16_t AccZOffset = -75;

int16_t GyroXOffset = 42;
int16_t GyroYOffset = -38;
int16_t GyroZOffset = 54;
...
void DataCollection(IMU *imu)
{
    ...
    battery = Get_Adc(8)/6;
    // rad/s   m/s^2  
    imu->acc_raw_x = (float)imu->acc_adc_x * ACC_SCALE * CONSTANTS_ONE_G ;
    imu->acc_raw_y = (float)imu->acc_adc_y * ACC_SCALE * CONSTANTS_ONE_G ;
    imu->acc_raw_z = (float)imu->acc_adc_z * ACC_SCALE * CONSTANTS_ONE_G ;

    imu->gyro_raw_x = (float)imu->gyro_adc_x * GYRO_SCALE * M_PI /180.f;
    imu->gyro_raw_y = (float)imu->gyro_adc_y * GYRO_SCALE * M_PI /180.f;
    imu->gyro_raw_z = (float)imu->gyro_adc_z * GYRO_SCALE * M_PI /180.f;    
    ...
    imu->acc_x = LPF2pApply_1(imu->acc_raw_x + imu->acc_offset_x);
    imu->acc_y = LPF2pApply_2(imu->acc_raw_y + imu->acc_offset_y);
    imu->acc_z = LPF2pApply_3(imu->acc_raw_z + imu->acc_offset_z);
    
    imu->gyro_x = LPF2pApply_4(imu->gyro_raw_x + imu->gyro_offset_x);
    imu->gyro_y = LPF2pApply_5(imu->gyro_raw_y + imu->gyro_offset_y);
    imu->gyro_z = LPF2pApply_6(imu->gyro_raw_z + imu->gyro_offset_z);
...
//        OutData[0] = MS5611_Temperature;
//        OutData[1] = MS5611_Altitude*100.0f;
//        OutData[2] = (MS5611_Pressure-101220)*10.0f;
//        OutData[3] = 0;
